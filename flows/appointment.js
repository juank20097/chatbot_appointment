...
const flowValidateDate2 = addKeyword([date])
  .addAction(async (ctx, { flowDynamic, gotoFlow }) => {
    ...
    if (!utilities.validateDateFormat(date)) {
      ...
      return gotoFlow(flowSelectDate);
    }
  ...
  .addAnswer(
    ...
    async (ctx, { flowDynamic, gotoFlow }) => {
      ...
      if (event) {
        ...
        await flowDynamic('😕 El número ingresado no corresponde a ninguna cita. Volvamos a intentar.');
        return gotoFlow(flowCambiar);
      }
    },
    [flowSelectDate]
  )
...
const flowCambiar = addKeyword(['3'])
  ...
  .addAnswer('Perfecto! vamos a realizar el cambio de la cita. ✍️')
  ...
  .addAction(async (ctx, { gotoFlow, flowDynamic }) => {
    ...
    try {
      ...
      if (data.items && data.items.length > 0) {
        ...
        const user = await serviceUser.getUserByDni(dni);
...
/*--------------------------------------------2) Verificar-------------------------------------------------------------*/
const flowValidateUser = addKeyword('')
  .addAction(async (ctx, { gotoFlow, flowDynamic }) => {
    const userId = ctx.from;
    startInactivityTimer(userId, gotoFlow, flowDynamic, ctx);
    await flowDynamic(
      `✅ *Has ingresado los siguientes datos:*\n\n` +
      `📇 *Identificación:* ${user.dni}\n` +
      `🙍‍♂️ *Nombre:* ${user.name}\n` +
      `📧 *Email:* ${user.email}`
    );
  })
  .addAnswer(
    ...
    async (ctx, { flowDynamic, gotoFlow, fallBack }) => {
      ...
      if (respuesta === '1' || respuesta.toLowerCase() === 'sí' || respuesta.toLowerCase() === 'si') {
        await flowDynamic('✅ *Datos correctos.* ¡Gracias por confirmar! 😊');
...
const flowEmail = addKeyword('')
  ...
  .addAnswer(
    `¡Gracias, ${user.name}! 🙏 Por favor, ingresa tu correo electrónico para enviarte los detalles de tu cita.`,
    { capture: true },
    async (ctx, { gotoFlow, flowDynamic }) => {
      const userId = ctx.from;
      startInactivityTimer(userId, gotoFlow, flowDynamic, ctx);
...
const flowCreate = addKeyword('')
  ...
  .addAnswer(
    `¿El dato que ingresaste es correcto?\n\n👉 *1.* Sí\n👉 *2.* No`,
    { capture: true },
    async (ctx, { flowDynamic, gotoFlow, fallBack }) => {
      const userId = ctx.from;
      startInactivityTimer(userId, gotoFlow, flowDynamic, ctx);
...
const flowDni = addKeyword(['1', '2', '3', '4', '5', '6', '7', '8', '9'])
  ...
  .addAnswer(['🌟 ¡Estamos casi listos!',
    '',
    'Por favor, ingresa tu cédula o pasaporte. 😊'
  ],
    { capture: true },
    async (ctx, { flowDynamic, gotoFlow }) => {
      const userId = ctx.from;
